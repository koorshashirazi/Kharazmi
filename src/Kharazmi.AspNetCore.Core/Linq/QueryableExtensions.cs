    ...
    public static class QueryableExtensions
    {
        ...
        private static IQueryable<T> Filters<T>(IQueryable<T> queryable, Filter filter, List<object> errors)
        {
            if (filter?.Logic != null)
            {
                ...
                // Step.2 Make up expression e.g. (p.Number >= 3) AndAlso (p.Company.AggregateType.Contains("M"))
                ...
                // Step.3 The result is e.g. p => (p.Number >= 3) AndAlso (p.Company.AggregateType.Contains("M"))
