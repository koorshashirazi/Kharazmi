...
namespace Kharazmi.AspNetCore.Core.Http
{
    ...
    public static class DownloaderServiceExtensions
    {
        ...
        {
            services.TryAddTransient<IDownloaderService, DownloaderService>();
            return services;
        }
    }
    ...
    public class DownloaderService : IDownloaderService
    {
        ...
        {
            var result = await DownloadDataAsync(url, autoRetries).ConfigureAwait(false);
        ...
        private async Task<T> downloadAsync<T>(Func<Task<T>> task, AutoRetriesPolicy autoRetries)
        {
            ...
            do
            {
                ...
                if (exceptions.Any())
                {
                    ...
                    await Task.Delay(autoRetries.AutoRetriesDelay, _cancelSrc.Token).ConfigureAwait(false);
                }
        ...
        private async Task<DownloadStatus> doDownloadFileAsync(string url, string outputFilePath)
        {
            ...
            var response = await readResponseHeadersAsync(url).ConfigureAwait(false);
            ...
            using (var inputStream = await response.Content.ReadAsStreamAsync().ConfigureAwait(false))
            {
                                                       ...
                                                       ))
                {
                    ...
                    await readInputStreamAsync(inputStream, fileStream).ConfigureAwait(false);
        ...
        private async Task readInputStreamAsync(Stream inputStream, Stream outputStream)
        {
            ...
            int read;
            var readCount = 0L;
            while ((read = await inputStream.ReadAsync(buffer, 0, buffer.Length, _cancelSrc.Token).ConfigureAwait(false)) > 0 &&
                   !_cancelSrc.IsCancellationRequested)
            {
                ...
                await outputStream.WriteAsync(buffer, 0, read, _cancelSrc.Token).ConfigureAwait(false);
            }
        }
        ...
        private async Task<HttpResponseMessage> readResponseHeadersAsync(string url)
        {
            ...
            var response = await _client.GetAsync(url, HttpCompletionOption.ResponseHeadersRead, _cancelSrc.Token).ConfigureAwait(false);
        ...
        private async Task<(byte[] Data, DownloadStatus DownloadStatus)> doDownloadDataAsync(string url)
        {
            ...
            using (var inputStream = await response.Content.ReadAsStreamAsync().ConfigureAwait(false))
            {
                ...
                {
                    await readInputStreamAsync(inputStream, outputStream).ConfigureAwait(false);
