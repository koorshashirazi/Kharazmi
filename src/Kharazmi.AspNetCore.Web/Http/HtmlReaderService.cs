...
namespace Kharazmi.AspNetCore.Web.Http
{
    ...
    public static class HtmlReaderServiceExtensions
    {
        ...
        {
            services.TryAddTransient<IHtmlReaderService, HtmlReaderService>();
            return services;
        }
    }
    ...
    public class HtmlReaderService : IHtmlReaderService
    {
        ...
        public HtmlDocument CreateHtmlDocument(string html)
        {
            ...
            if (doc.ParseErrors != null && doc.ParseErrors.Any())
            {
                ...
                {
                    _logger.LogWarning($"LoadHtml Error. SourceText: {error.SourceText} -> NotificationId: {error.Code} -> Reason: {error.Reason}");
                }
            }
