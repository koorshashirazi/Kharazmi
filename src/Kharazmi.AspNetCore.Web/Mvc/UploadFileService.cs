...
namespace Kharazmi.AspNetCore.Web.Mvc
{
    ...
    internal class UploadFileService : IUploadFileService
    {
        ...
        /// <param name="allowOverwrite">Creates a unique file name if the file already exists.</param>
        /// <returns></returns>
        public async Task<bool> SavePostedFileAsync(IFormFile formFile, string destinationDirectoryName,
            bool allowOverwrite)
        {
        ...
        private static string GetUniqueFilePath(IFormFile formFile, string uploadsRootFolder)
        {
            ...
            return Path.Combine(uploadsRootFolder,
                $"{fileName}.{DateTime.Now:yyyyMMddHHmmssfff}.{Guid.NewGuid():N}{extension}");
        }
    }
}