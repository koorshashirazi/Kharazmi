...
namespace Kharazmi.AspNetCore.Web.TagHelpers
{
    ...
    public class PagerTagHelper : TagHelper
    {
        ...
        private readonly IBuildPaginationLinks linkBuilder;
        ...
        /// <summary>
        ///     An alternative to ControllerType/Action or Route
        /// </summary>
        /// <remarks>
        ...
        public override void Process(TagHelperContext context, TagHelperOutput output)
        {
            ...
            var totalPages = (long) Math.Ceiling(PagingModel.TotalItems / (double) PagingModel.ItemsPerPage);
            ...
            var links = linkBuilder.BuildPaginationLinks(
                PagingModel,
                GeneratePageUrl,
                FirstPageText,
                FirstPageTitle,
            ...
            foreach (var link in links)
            {
                ...
                else
                {
                    if (!link.IsCurrent && link.Active)
                    {
                        ...
                        else // if text should be html encoded
        ...
        private string GeneratePageUrl(long pageNumber)
        {
            ...
            var blockedQuery = Enumerable.Empty<KeyValuePair<string, StringValues>>();
            if (PreserveAmbientQueryString) blockedQuery = AddCurrentQueryString(routeValues);


            if (Route != null)
                return AppendBlockedQuery(urlHelper.Link(Route, routeValues),
                    blockedQuery); // endpoint route return null when no route be found
            ...
            if (BaseHref != null)
            {
                ...
                return
                    $"{BaseHref}{start}{routeValues.Select(x => $"{x.Key}={x.Value}").Aggregate((current, next) => $"{current}&{next}")}";
            }
            ...
            return AppendBlockedQuery(urlHelper.Action(Action, Controller, routeValues),
                blockedQuery); // endpoint route return null when no route be found
        }
        ...
        private IEnumerable<KeyValuePair<string, StringValues>> AddCurrentQueryString(
            Dictionary<string, object> current)
        {
            ...
            foreach (var item in urlHelper.ActionContext.HttpContext.Request.Query)
                ...
                else if (!current.ContainsKey(item.Key)) current.Add(item.Key, item.Value);
            return blocked;
        }

        private string AppendBlockedQuery(string url, IEnumerable<KeyValuePair<string, StringValues>> blockedQuery)
        {
            if (string.IsNullOrEmpty(url)) return string.Empty; // fix NullReferenceException
            if (blockedQuery == null && !blockedQuery.Any()) return url ?? string.Empty;
            ...
            if (string.IsNullOrEmpty(queryString)) return url ?? string.Empty;
            if (url.Contains("?"))
