...
namespace Kharazmi.AspNetCore.EFCore.Context.Extensions
{
    public static class UnitOfWorkExtensions
    {
        ...
        {
            var entity = await uow.Set<TEntity>().FindAsync(id).ConfigureAwait(false);
        ...
        {
            var tamperedEntryList = await uow.TamperedEntryListAsync(predicate).ConfigureAwait(false);
        ...
        {
            foreach (var entity in entities)
                await uow.LoadRelatedEntitiesAsync(entity).ConfigureAwait(false);
        }
