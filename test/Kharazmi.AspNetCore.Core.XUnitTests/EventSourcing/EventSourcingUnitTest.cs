...
using Kharazmi.AspNetCore.Core.XUnitTests.Domain.Aggregates.Events;
...
namespace Kharazmi.AspNetCore.Core.XUnitTests.EventSourcing
{
    ...
    public class EventSourcingUnitTest
    {
        ...
        public void GetUncommittedEvents_AfterEmittingEvents_ReturnsAllEvents()
        {
            ...
            // Assert
            AssertionExtensions.Should((int)events.Count).Be(2);
            Enumerable.Select<IDomainEvent, string>(events, e => ((TestEvent)e).Data).Should().BeEquivalentTo("Event1", "Event2");
        }
        ...
        public async Task ApplyFromHistoryAsync_WithValidEventStore_AppliesEvents()
        {
            ...
            mockEventStore.Setup<Task<IReadOnlyCollection<IDomainEvent>>>(es => es.GetEventsAsync<TestAggregate, Guid>(aggregate.Id, It.IsAny<CancellationToken>()))
                .ReturnsAsync(events);
            ...
            // Assert
            AssertionExtensions.Should((ulong)aggregate.Version).Be(2);
            AssertionExtensions.Should((string)aggregate.Data).Be("HistoricalEvent2"); // Last event applied
            mockEventStore.Verify<Task<IReadOnlyCollection<IDomainEvent>>>(
        ...
        public async Task CommitAsync_WithValidEventStore_SavesEvents()
        {
            ...
            var mockEventStore = new Mock<IEventStore>();
            mockEventStore.Setup<Task>(es => es.SaveAsync(It.IsAny<TestAggregate>(), It.IsAny<CancellationToken>()))
